...
# Collector password is stored in Streamlit Cloud Secrets, never in GitHub.
...
is_collector = bool(collector_secret) and hmac.compare_digest(
    collector_password, collector_secret
)
if is_collector:
...
lap_number_raw = feed.get("lap_number", "-")
laps_to_go_raw = feed.get("laps_to_go", "-")
stage_num = feed.get(
    "stage_num",
    feed.get("stage", {}).get("stage_num", "-")
    if isinstance(feed.get("stage"), dict)
    else "-",
)
flag_state = feed.get("flag_state", "-")

try:
    current_lap = int(lap_number_raw)
...
st.markdown(
    ...
    <div class="status-row">
      <div class="status-item"><span class="status-label">Lap</span><span class="status-value">{lap_number_raw}</span></div>
...
for vehicle in vehicles:
    raw_name = (
        ...
        or vehicle.get("driver", {}).get("full_name")
        or vehicle.get("full_name")
        or "Unknown"
    )
...
draft_rank_by_key = {
    row["_key"]: index + 1 for index, row in enumerate(team_rows_sorted)
}
for row in rows:
...
if not df.empty:
    ...
    df["Live Win Odds"] = df["Live Win Odds"].apply(
...
# ---------------------------------------------------------------------------
# Persistent JSON history. Only the password-protected desktop session writes.
# ---------------------------------------------------------------------------
history = load_history()
is_final_lap = laps_to_go == 0
should_record = current_lap is not None and (
    current_lap == 1 or current_lap % 10 == 0 or is_final_lap
)
...
if history:
    ...
    history_laps = [int(item["lap"]) for item in history]

    for index, (team, _) in enumerate(team_map.values()):
        points = [
            (int(item["lap"]), item["positions"][team])
            for item in history
            if team in item.get("positions", {})
        ]
        if not points:
            continue
        xs, ys = zip(*points)
...
st.caption(
    f"Last refreshed: {datetime.now().strftime('%H:%M:%S')} — auto-refreshing every {REFRESH_SECONDS}s"
)
