...
# ---------------------------------------------------------------------------
# NASCAR's feed leaks entry-list annotation markers into driver_name:
#   "#"    -> rookie
#   "(i)"  -> ineligible for series points (mid-season subs, open entries)
#   "*"    -> open/non-charter entry flag (usually leading)
# Strip all of these before displaying or using the name as a lookup key.
# ---------------------------------------------------------------------------
def clean_feed_name(raw: str) -> str:
    n = raw.strip()
    changed = True
    while changed:
        changed = False
...
def normalize(name: str) -> str:
    n = clean_feed_name(name).lower().replace(".", "")
    n = ALIASES.get(n, n)
...
if not df.empty:
    ...
    if "Live Win Odds" in df.columns:
        df["Live Win Odds"] = df["Live Win Odds"].apply(lambda x: f"{x*100:.1f}%" if pd.notnull(x) else "—")
